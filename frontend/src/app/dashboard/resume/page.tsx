...
import { 
  ...
  ArrowRight, BookText, Crown, Zap, RotateCcw
} from 'lucide-react';
...
const ForensicLoader = ({ active }: { active: boolean }) => {
  ...
  const steps = [
    ...
    "Extracting Technical & Growth Signals...",
    "Finalizing Forensic ATS Index..."
  ];
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0f1117]/95 backdrop-blur-3xl flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full space-y-12">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <h2 className="text-2xl font-black font-outfit tracking-tight text-white">Forensic Audit Active</h2>
          <p className="text-sm text-white/40 font-medium tracking-wide">Simulating recruitment AI decision-tree analysis...</p>
        </div>
          ...
          {steps.map((text, idx) => (
            ...
            >
              <div className={cn(
                ...
                step > idx ? "bg-emerald-500/10 border-emerald-500 shadow-[0_0_10px_rgba(34,197,94,0.3)]" : 
                step === idx ? "bg-primary/10 border-primary animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.3)]" : "border-white/10"
              )}>
                {step > idx ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <div className="w-1.5 h-1.5 rounded-full bg-current" />}
              </div>
              <span className={cn(
                "text-sm font-black uppercase tracking-[0.15em] transition-all duration-500",
                step === idx ? "translate-x-2" : ""
              )}>
                {text}
              </span>
            </motion.div>
